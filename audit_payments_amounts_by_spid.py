...
print(f"{'SP_ID':<8} | {'OP':<10} | {'SP_Importe':>15} | {'DB_Importe':>15} | {'Diff':>12}")
print("-" * 75)
...
for i in items:
    ...
    # Check in DB by sp_id
    db_res = supabase.table("pagos").select("importe_pago, op_numero").eq("sp_id", sp_id).execute()
    if db_res.data:
        db_val = float(db_res.data[0]["importe_pago"] or 0)
        diff = sp_val - db_val
        print(f"{sp_id:<8} | {str(sp_op):<10} | {sp_val:>15.2f} | {db_val:>15.2f} | {diff:>12.2f}")
    else:
        print(f"{sp_id:<8} | {str(sp_op):<10} | {sp_val:>15.2f} | {'Missing':>15} | {'-':>12}")
