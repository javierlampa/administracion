...
# 1. CARGAR EXCEL (EL QUE ME PASASTE)
print("--- CARGANDO EXCEL (csv/tv.xlsx) ---")
...
# Agrupar Excel por OP y Programa para comparar peras con peras
excel_grouped = df_excel.groupby(['OP_Limpia', 'Programas.lookupValue'])['Importe Total'].sum().reset_index()

# 2. CARGAR BASE DE DATOS (2026 en el mismo rango)
print("--- CARGANDO BASE DE DATOS (Rango 2026-01-01 al 2026-04-14) ---")
...
# 3. CRUZAR DATOS PARA ENCONTRAR DIFERENCIAS
merged = pd.merge(
    db_grouped, 
    excel_grouped, 
...
merged['importe_DB'] = merged['importe_DB'].fillna(0)
merged['Importe Total'] = merged['Importe Total'].fillna(0)
merged['diff'] = merged['importe_DB'] - merged['Importe Total']

# Solo mostrar diferencias grandes (más de 1 peso)
diferencias = merged[abs(merged['diff']) > 1].sort_values('diff', ascending=False)

print(f"\nTOTAL DB EN RANGO: {df_db['importe'].sum():,.2f}")
print(f"TOTAL EXCEL RECIBIDO: {df_excel['Importe Total'].sum():,.2f}")
print(f"DIFERENCIA TOTAL: {df_db['importe'].sum() - df_excel['Importe Total'].sum():,.2f}")

print("\n--- DETALLE DE OPS QUE NO COINCIDEN ---")
print(diferencias[['OP_Limpia', 'programa', 'importe_DB', 'Importe Total', 'diff']].to_string(index=False))
